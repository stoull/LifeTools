...
# 基于 https://github.com/liangxiaobo/imageCompression 更改
# 当使用-d 且-o时，压缩后会覆盖原来的图片如：
# python3 ./imageCompression-override.py -d ./ -q 80 -o True
...
def main(argv):
    ...
    override = False # 是否要覆盖原来的文件

    try:
        opts, args = getopt.getopt(argv, "hi:q:s:j:d:o:", ["imgs=", "quality=", "subsampling=", "jpga=", "dir", "override"])
    ...
    if dir_files:
        dirOfImageCompress(dir_files, quality, subsampling, notfound_imgs, jpga, output, override)
        return
...
def dirOfImageCompress(dir, quality, subsampling, notfound_imgs, jpga, output, override):
    '''
    ...
    for dirpath, dirname, filenames in os.walk(dir):
        ...
        if dirpath.endswith('/output') == False:
            ...
            if override:
                # 覆盖原来的image
                output_dir = Path('{}'.format(dirpath))
...
def imageCompress(quality, subsampling, img_item, notfound_imgs, jpga, output):
    ...
    if img_item_path.is_file() and img_item_endswith:
        ...
        img.save(img_item_path, quality=quality, optimize=True, subsampling=subsampling)
